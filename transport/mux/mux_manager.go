...
type (
	...
	MuxManager interface {
		...
		GetConnectionInfo(name string) []common.ConnectionInfo
	}
)
...
// GetConnectionInfo returns debug information about this connection manager
func (m *muxManager) GetConnectionInfo(name string) []common.ConnectionInfo {
	// Best-effort: avoid blocking callers (e.g., debug endpoint) if the manager is busy
	// Cast to *sync.Mutex to access TryLock method
	mutex, ok := m.connAvailable.L.(*sync.Mutex)
	if !ok || !mutex.TryLock() {
	...
	var streams int
	var activeStreams []common.StreamInfo

	// Determine status based on shutdown state and connection availability
	if m.hasShutDown.IsShutdown() {
		statusStr = "stopped"
	} else if m.shouldShutDown.IsShutdown() {
		statusStr = "stopping"
	} else {
		// Check if we have an active connection
		swc := m.muxConnection.Load()
		if swc != nil && !swc.IsClosed() {
			connected = true
			statusStr = "connected"
			if swc.Conn != nil {
				localAddr = swc.Conn.LocalAddr().String()
				remoteAddr = swc.Conn.RemoteAddr().String()
			}
			if swc.Session != nil {
				streams = swc.Session.NumStreams()
			}
		} else {
			// Check if manager has been started (init.Do would have been called)
	...
	connections = append(connections, common.ConnectionInfo{
		Name:          name,
		Type:          "mux",
		Status:        statusStr,
		LocalAddr:     localAddr,
