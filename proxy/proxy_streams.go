...
func (r *proxyStreamReceiver) Run(
	...
	var err error
	if r.proxyServer.opts.IsInbound {
		sourceStreamClient, err = r.proxy.outboundServer.GetAdminClient().StreamWorkflowReplicationMessages(outgoingContext)
	} else {
		sourceStreamClient, err = r.proxy.inboundServer.GetAdminClient().StreamWorkflowReplicationMessages(outgoingContext)
	}
	if err != nil {
...
func (f *proxyStreamForwarder) Run(
	...
	var wg sync.WaitGroup
	wg.Add(2)
	go f.forwardAck(streamTracker, sourceStreamServer, sourceStreamClient, shutdownChan, &wg)
	go f.forwardReplicationMessages(streamTracker, sourceStreamServer, sourceStreamClient, shutdownChan, &wg)
	wg.Wait()
}
...
func (f *proxyStreamForwarder) forwardAck(
	...
	f.logger.Info("proxyStreamForwarder forwardAck started")
	...
	dataChan := startListener(sourceStreamServer, shutdownChan)
		...
		case *adminservice.StreamWorkflowReplicationMessagesRequest_SyncReplicationState:
			...
			if err = sourceStreamClient.Send(req); err != nil {
				if err != io.EOF {
					...
					f.logger.Info("sourceStreamClient.Send encountered EOF", tag.Error(err))
				}
				return
			}
		default:
		...
		case *adminservice.StreamWorkflowReplicationMessagesResponse_Messages:
			...
			if err = sourceStreamServer.Send(resp); err != nil {
				if err != io.EOF {
					...
					f.logger.Info("targetStreamServer.Send encountered EOF", tag.Error(err))
				}
				return
			}
		default:
