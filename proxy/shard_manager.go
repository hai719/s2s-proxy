...
func (sm *shardManagerImpl) Start() error {
...
func (sm *shardManagerImpl) DeliverAckToShardOwner(
	...
	if owner, ok := sm.GetShardOwner(sourceShard); ok && owner != sm.config.NodeName {
		if addr, found := sm.GetProxyAddress(owner); found {
			...
			mgr := proxy.GetIntraProxyManager()
			...
			if err := mgr.sendAck(context.Background(), owner, clientShard, serverShard, proxy, routedAck.Req); err != nil {
				...
				if sm.config != nil {
					metrics.ShardForwardingCounter.WithLabelValues(sm.config.NodeName, owner, "ack_error").Inc()
				}
				return false
			}
			metrics.ShardForwardingCounter.WithLabelValues(sm.config.NodeName, owner, "ack_remote").Inc()
...
func (sm *shardManagerImpl) DeliverMessagesToShardOwner(
	...
	if sm.config != nil {
		if owner, ok := sm.GetShardOwner(targetShard); ok && owner != sm.config.NodeName {
			if addr, found := sm.GetProxyAddress(owner); found {
				if mgr := proxy.GetIntraProxyManager(); mgr != nil {
					resp := routedMsg.Resp
					if err := mgr.sendReplicationMessages(context.Background(), owner, targetShard, routedMsg.SourceShard, proxy, resp); err != nil {
						...
						if sm.config != nil {
							metrics.ShardForwardingCounter.WithLabelValues(sm.config.NodeName, owner, "msg_error").Inc()
						}
						return false
					}
					if sm.config != nil {
						metrics.ShardForwardingCounter.WithLabelValues(sm.config.NodeName, owner, "msg_remote").Inc()
					}
					return true
				}
			} else {
...
func (sed *shardEventDelegate) NotifyJoin(node *memberlist.Node) {
	...
	metrics.MemberlistEventsCounter.WithLabelValues("join").Inc()
	metrics.MemberlistClusterSizeGauge.Set(float64(len(sed.manager.GetMemberNodes())))

	// Invoke callback if registered
	if sed.manager != nil && sed.manager.onPeerJoin != nil {
		sed.manager.onPeerJoin(node.Name)
	}
}
...
func (sed *shardEventDelegate) NotifyLeave(node *memberlist.Node) {
	...
	metrics.MemberlistEventsCounter.WithLabelValues("leave").Inc()
	metrics.MemberlistClusterSizeGauge.Set(float64(len(sed.manager.GetMemberNodes())))

	if sed.manager != nil && sed.manager.onPeerLeave != nil {
		sed.manager.onPeerLeave(node.Name)
	}
}
