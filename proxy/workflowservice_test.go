...
func (s *workflowServiceTestSuite) TestNamespaceFiltering() {
	...
	ps := &ProxyServer{}
	wfProxy := NewWorkflowServiceProxyServer(ps, "My cool test server", s.clientConfig, s.clientFactoryMock,
...
func (s *workflowServiceTestSuite) TestPreserveRedirectionHeader() {
	wfProxy := NewWorkflowServiceProxyServer(&ProxyServer{}, "My cool test server", s.clientConfig, s.clientFactoryMock, nil, log.NewTestLogger())
